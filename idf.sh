...
case "$command" in
  ...
  build)
    ...
    ;;
  work)
    docker run -it --rm --device=/dev/ttyUSB0:/dev/ttyUSB0 -v $PWD/$TARGET_PROJECT:/project -w /project -e HOME=/tmp espressif/idf:$IDF_VER bash
    ;;
  flash)
    ...
    docker run --rm --device=/dev/ttyUSB0:/dev/ttyUSB0 -v $PWD/$TARGET_PROJECT:/project -w /project -e HOME=/tmp --entrypoint="" espressif/idf:$IDF_VER bash -c "$cmd"
    ;;
  monitor)
    docker run -it --rm --device=/dev/ttyUSB0:/dev/ttyUSB0 -v $PWD/$TARGET_PROJECT:/project -w /project -e HOME=/tmp espressif/idf:$IDF_VER idf.py monitor
    ;;
  *)
